...
void Server::parseLocationBlock(std::string &locationBlock) {
    ...
    if (locationString[1] != "=" && locationString[1][0] != '/')
        throw std::runtime_error("Location urlType is not validate");

    loc.urlType = locationString[1];
...
bool Server::settingLocationBlock(std::vector<std::string>& strs, size_t size, Location &loc) {
    if (strs[0] == "root") {
        ...
        if (strs[size - 1].back() != ';')
            throw std::runtime_error("Missing semicolon in 'autoindex' directive");
        if (size != 2)
            throw std::runtime_error("autoindex must have only one number");
        strs[1].erase(strs[1].size() - 1);
        if (strs[1] == "on")
            loc.autoIndex = true;
        else
